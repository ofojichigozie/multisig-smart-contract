...
contract MultiSigWallet {
    ...
    modifier onlyWallet() {
        require(msg.sender == address(this),
            "MultiSigWallet: can only be called by multisig wallet");
        _;
    }

    modifier ownerDoesNotExist(address owner) {
        require(!isOwner[owner],
            "MultiSigWallet: owner does exist already");
        _;
    }
    ...
    modifier notConfirmed(uint256 transactionId, address owner) {
        require(!confirmations[transactionId][owner],
            "MultiSigWallet: transaction is confirmed already");
        _;
    }

    modifier notExecuted(uint256 transactionId) {
        require(!transactions[transactionId].executed,
            "MultiSigWallet: transaction is executed already");
        _;
    }

    modifier notNull(address _address) {
        require(_address != address(0),
            "MultiSigWallet: address is the zero address");
        _;
    }
    ...
    modifier validRequirement(uint256 ownerCount, uint256 _required) {
        require(ownerCount <= MAX_OWNER_COUNT
            ...
            && _required != 0
            && ownerCount != 0,
            "MultiSigWallet: requirement is not met");
        _;
    }
    ...
    /// @dev Makes an exteranl call to execute transaction.
    /// @param destination Transaction target address.
    /// @param value Transaction ether value.
    /// @param data Transaction data payload.
